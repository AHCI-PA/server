...
print(f"The device used is: {'cuda' if torch.cuda.is_available() else 'cpu'}!")
...
size = int(conf_parser["shap-e"]["size"])  # Size of the created models
...
def cache_purge():
    ...
    cache_dir = os.getcwd() + conf_parser["DEFAULT"]["cache_subdir"]
...
def export_to_video(images, filepath):
    video_dimension = (images[0].size)
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    FPS = 10
    video = cv2.VideoWriter(filepath, fourcc, FPS, video_dimension)
    for image in images:
...
def cache_update(models, query, cli_id):
    ...
    # Create directories for storing GIFs/Videos and meshes
    ...
    # Generate GIFs/Videos and meshes for each model and save them to the cache directory
    for index, img in enumerate(models):
        # Generate GIF/Vdeos from model data

        export_to_video(gif_files[index], f"{cache_dir}{cli_id}/{query}/gifs/{index}_model.mp4")
    ...
    # Compress GIFs/Videos into a ZIP file
...
# Setup for flask
app = Flask(__name__)
port = conf_parser["ngrok"]["port"]

# The Authentication Token is given to Ngrok
conf.get_default().authtoken = "2eJaMvAvIJVo3sXLNB7CyQepsx8_6mtU2mZ91hBxzeJv5HNt6"
...
def deauthorize():
    ...
    # Check if client ID is authorized
    if cli_id in clients:
        del clients[cli_id]
        # Client is no longer authorized
        return '', 200
    else:
        return 'The client was already unauthorized', 401 # Unauthorized
...
def pictures_exchange():
    ...
    translated_query = translate(query) if conf_parser["DEFAULT"]["language"] != "en" else query
    tentatives = int(parse.unquote(request.args["tentatives"]))

    # Generate models based on the query
    models = query_to_model(translated_query, tentatives)

    # Update cache with generated models
    cache_update(models, query, cli_id)

    end_time = time.time()
    print(f"Request processed in: {end_time - start_time}")
